...
階段 4 已在既有 Discord 訊息、安全閘門、SQLite 持久化、確定性切段與一次性預算帳本之上，
加入 channel ID 模式、免費回覆觸發、版本化人設、有限上下文及 OpenAI Responses API 回覆。
程式在沒有 Discord 設定時仍以安全模式啟動；設定完整時才啟動 Discord.py 用戶端。

本階段不包含摘要、Embedding、歷史匯入、提醒或部署。沒有 OpenAI 金鑰時只回固定維護訊息；
設定金鑰後，符合觸發條件的訊息可能產生受預算帳本限制的付費呼叫。
...
2. 優先加入明確回覆鏈，再補目前段落近期訊息，合計最多 12,000 字元。
