...
- `app.history_cli`：不含正式匯入動作的免費分析命令列入口。
...
費用報告同時計入新歷史估計段落，以及白名單頻道中已封存但尚未摘要的既有段落。摘要輸入
以 UTF-8 位元組與固定提示餘裕作 Token 保守上界，並逐次套用整數微美元向上取整；Embedding
以上限 300 摘要 Token 估算。報告讀取預算快照，但不呼叫 `reserve`，因此不會建立付費紀錄。

正式匯入不與 `analyze` 共用隱含開關。後續只有取得最新報告的二次確認後才能提供寫入命令；
屆時 Discord message ID 唯一鍵、階段 5 工作冪等鍵與持久化進度共同確保安全重跑。
